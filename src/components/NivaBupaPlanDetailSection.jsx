...
// Compact Feature-Wise Inline Video Button Component (Niva Bupa Cyan/Sky Blue Theme)
const VideoButton = ({ featureTitle, onOpenVideo, videoUrl }) => {
  return (
    <button
      ...
      className="inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[11px] font-bold bg-[#F0F9FF] text-[#0284C7] border border-[#0EA5E9]/25 hover:bg-[#0EA5E9] hover:text-white transition-all cursor-pointer select-none shrink-0 shadow-2xs group align-middle ml-0.5 sm:ml-1"
...
// Premium "WATCH VIDEO" button — matches Report Card & Modal design (Niva Bupa Theme)
const WatchVideoButton = ({ title, onOpenVideo, videoUrl, className = '', align = 'center' }) => (
  <div className={`pt-1.5 border-t border-slate-100/80 ${align === 'center' ? 'flex justify-center' : ''} ${className}`}>
    <button
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] sm:text-xs font-bold bg-white text-[#0284C7] border border-[#0EA5E9]/25 hover:bg-[#0EA5E9] hover:text-white transition-all cursor-pointer shadow-2xs group select-none"
    >
      <FiPlay className="text-[9px] sm:text-[10px] fill-current text-[#0284C7] group-hover:text-white transition-colors" />
      <span>WATCH VIDEO</span>
    </button>
  </div>
);

// Premium In-Page Video Lightbox Modal (Niva Bupa Theme)
            ...
            <div className="px-2.5 pb-2.5 sm:px-4.5 sm:pb-4.5 border-t border-slate-100/80 text-slate-600 space-y-2 sm:space-y-2.5">
              ...
              {badge && (
                <div className="pt-2 sm:pt-3 flex flex-wrap items-center gap-1.5 sm:gap-2">
                  <span className="inline-flex items-center gap-1 text-[8px] sm:text-[10px] font-black uppercase px-2 py-0.5 rounded-md bg-[#F0F9FF] text-[#0284C7] border border-[#0EA5E9]/20 tracking-wider">
                  ...
                  <div className="flex flex-wrap items-center gap-1 sm:gap-2">
                    {steps.map((step, sIdx) => (
                      <React.Fragment key={sIdx}>
                        ...
                        {sIdx < steps.length - 1 && (
                          <span className="text-[10px] sm:text-xs font-extrabold text-[#0284C7] px-0.5">
                            →
                          </span>
                        )}
                      </React.Fragment>
...
export default function NivaBupaPlanDetailSection({ plan, company, planId: planIdProp }) {
  ...
  // Resolve to canonical Niva Bupa plan ID — each plan gets independent data
  const currentPlanId = resolveNivaBupaPlanId(planIdProp || plan?.id || urlPlanId);
  const planData = getNivaBupaPlanData(currentPlanId);
  ...
  // =========================================================================
  // MAIN NIVA BUPA PLAN DETAIL PAGE (SINGLE VIEWPORT — APPROVED HDFC UX STRUCTURE)
  // =========================================================================
  return (
    <div className="w-full">
      ...
      <div className="max-w-3xl mx-auto flex flex-col justify-start sm:justify-center items-stretch sm:min-h-[calc(100vh-220px)] py-1 sm:py-4 space-y-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-4 md:gap-5 w-full">
          ...
          <button
            ...
            className={`bg-white rounded-xl sm:rounded-2xl border p-2.5 sm:p-4 md:p-5 flex items-center justify-between text-left shadow-2xs hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer group relative overflow-hidden active:scale-[0.98] select-none ${
              activeModal === 'ratio'
                ...
                : 'border-slate-200/80 hover:border-[#0EA5E9]/40'
            }`}
          >
            <div className={`absolute bottom-0 left-0 right-0 h-[2.5px] transition-colors duration-200 ${
              activeModal === 'ratio' ? 'bg-[#0EA5E9]' : 'bg-[#0EA5E9]/30 group-hover:bg-[#0EA5E9]'
            }`} />
            <h3 className="text-xs sm:text-base font-extrabold text-[#0F172A] group-hover:text-[#0284C7] transition-colors duration-200 font-display leading-tight pr-1 tracking-tight">
              REPORT CARD
            </h3>
          ...
          >
            <div className={`absolute bottom-0 left-0 right-0 h-[2.5px] transition-colors duration-200 ${
              activeModal === 'fundamental' ? 'bg-[#0EA5E9]' : 'bg-[#0EA5E9]/30 group-hover:bg-[#0EA5E9]'
            }`} />
            <h3 className="text-xs sm:text-base font-extrabold text-[#0F172A] group-hover:text-[#0284C7] transition-colors duration-200 font-display leading-tight pr-1 tracking-tight">
              COMPANY STRENGTH
            </h3>
          ...
          >
            <div className={`absolute bottom-0 left-0 right-0 h-[2.5px] transition-colors duration-200 ${
              activeModal === 'limitations' ? 'bg-[#0EA5E9]' : 'bg-[#0EA5E9]/30 group-hover:bg-[#0EA5E9]'
            }`} />
            <h3 className="text-xs sm:text-base font-extrabold text-[#0F172A] group-hover:text-[#0284C7] transition-colors duration-200 font-display leading-tight pr-1 tracking-tight">
          ...
          >
            <div className={`absolute bottom-0 left-0 right-0 h-[2.5px] transition-colors duration-200 ${
              activeModal === 'mustKnow' ? 'bg-[#0EA5E9]' : 'bg-[#0EA5E9]/30 group-hover:bg-[#0EA5E9]'
            }`} />
            <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 pr-1">
              <motion.span
                animate={{ scale: [1, 1.15, 1], opacity: [0.85, 1, 0.85] }}
                transition={{ duration: 2.8, repeat: Infinity, ease: "easeInOut" }}
                className="text-[#0EA5E9] text-xs sm:text-base font-black select-none shrink-0"
              >
                ✦
              </motion.span>
              <h3 className="text-xs sm:text-base font-extrabold text-[#0F172A] group-hover:text-[#0284C7] transition-colors duration-200 font-display leading-tight tracking-tight uppercase">
                MUST KNOW DETAILS
              </h3>
            </div>
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:text-[#0284C7] group-hover:bg-[#F0F9FF] group-hover:border-[#0EA5E9]/20 transition-all duration-200 shrink-0">
          ...
          <button
            ...
            className={`bg-white rounded-xl sm:rounded-2xl border p-2.5 sm:p-4 md:p-5 flex items-center justify-between text-left shadow-2xs hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer group relative overflow-hidden active:scale-[0.98] select-none ${
              activeModal === 'bestSuitedFor'
                ...
                : 'border-slate-200/80 hover:border-[#00A8B5]/40'
            }`}
          >
            <div className={`absolute bottom-0 left-0 right-0 h-[2.5px] transition-colors duration-200 ${
              activeModal === 'bestSuitedFor' ? 'bg-[#00A8B5]' : 'bg-[#00A8B5]/30 group-hover:bg-[#00A8B5]'
            }`} />
            <h3 className="text-xs sm:text-base font-extrabold text-[#0F172A] group-hover:text-[#00A8B5] transition-colors duration-200 font-display leading-tight pr-1 tracking-tight uppercase">
              PERFECT FOR
            </h3>
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:text-[#00A8B5] group-hover:bg-[#E6F7F8] group-hover:border-[#00A8B5]/20 transition-all duration-200 shrink-0">
              <FiArrowRight className="text-xs sm:text-sm group-hover:translate-x-0.5 transition-transform duration-200" />
            </div>
          </button>
        </div>
            ...
            >
                  ...
                  if (tab.isLink) {
                    return (
                      <Link
                        key={tab.id}
                        to={tab.url}
                        className="px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-bold text-slate-600 hover:text-[#0284C7] hover:bg-[#F0F9FF] border border-transparent whitespace-nowrap transition-colors select-none"
                      >
                        {tab.label}
                      </Link>
                    );
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-bold whitespace-nowrap transition-all cursor-pointer select-none ${
                        isActive
                          ...
                          : 'text-slate-600 hover:text-[#0284C7] hover:bg-[#F0F9FF]'
                      }`}
                    >
                      {tab.label}
                    </button>
              ...
              {activeModal === 'ratio' && (
                <div className="space-y-4 sm:space-y-5">
                  ...
                  {/* 3 EQUAL ACCORDION BOXES — NIVA BUPA CYAN BORDERS & ACCENTS */}
                  <div className="space-y-2.5 sm:space-y-3">
                    {/* Box 1: CSR */}
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      <button
                        type="button"
                      ...
                      >
                        <div className="flex items-center justify-between flex-1 min-w-0 pr-2 sm:pr-3 gap-2">
                          <span className="text-xs sm:text-sm font-semibold tracking-wider uppercase text-slate-900 group-hover:text-[#0284C7] transition-colors font-display shrink-0">
                            CSR
                          </span>
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedReportCard.csr ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                        ...
                        {expandedReportCard.csr && (
                          ...
                          >
                            <div className="p-3.5 sm:p-4 border-t border-slate-100 bg-slate-50/30 space-y-3">
                              <div>
                                <span className="text-[11px] sm:text-xs font-semibold text-[#0284C7] tracking-wide block font-display">
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedReportCard.icr ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        <div className="flex items-center justify-between flex-1 min-w-0 pr-2 sm:pr-3 gap-2">
                          <span className="text-xs sm:text-sm font-semibold tracking-wider uppercase text-slate-900 group-hover:text-[#0284C7] transition-colors font-display truncate">
                            COMPLAINT VOLUME
                          </span>
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedReportCard.complaint ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
              ...
              {activeModal === 'fundamental' && (
                <div className="space-y-4 sm:space-y-5">
                  ...
                  {/* 6 EQUAL ACCORDION BOXES — NIVA BUPA THEME */}
                  <div className="space-y-2.5 sm:space-y-3">
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedCompanyStrength.ownership ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedCompanyStrength.creditRating ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedCompanyStrength.capitalStrength ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedCompanyStrength.financialBase ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedCompanyStrength.reinsurance ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
                    ...
                    <div className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors">
                      ...
                      >
                        ...
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-[#F0F9FF]/60 group-hover:border-[#0EA5E9]/25 transition-all duration-300 shrink-0 select-none">
                          <FiChevronDown className={`text-xs sm:text-sm transition-transform duration-300 transform ${expandedCompanyStrength.marketPosition ? 'rotate-180 text-[#0284C7]' : 'text-slate-400 group-hover:text-slate-600'}`} />
                        </div>
                      </button>
              ...
              {activeModal === 'limitations' && (
                <div className="space-y-4 sm:space-y-5">
                  <div className="pr-8">
                    <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-[#0284C7] block font-display">
                  ...
                  <div className="space-y-2.5 sm:space-y-3">
                    {planData.limitationsWaitingPeriods?.items.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="rounded-xl sm:rounded-2xl border border-[#0EA5E9]/35 bg-white overflow-hidden shadow-2xs hover:border-[#0EA5E9]/70 transition-colors"
                        >
                          <button
                            type="button"
                            ...
                            className="w-full p-3.5 sm:p-4 bg-white hover:bg-slate-50/50 flex items-center justify-between text-left transition-colors cursor-pointer select-none group gap-2"
                          >
                            <span className="text-xs sm:text-sm font-semibold tracking-wider uppercase text-slate-900 group-hover:text-[#0284C7] transition-colors font-display pr-2">
                              {item.title}
                            </span>
                            ...
                            {isItemExpanded && (
                              ...
                              >
                                <div className="p-3.5 sm:p-4 border-t border-slate-100 bg-slate-50/30 space-y-3">
                                  ...
                                  {item.diseaseList && (
                                    <div className="p-3 sm:p-4 rounded-xl bg-[#F0F9FF]/60 border border-[#0EA5E9]/15 space-y-2">
                                      <span className="text-[10px] sm:text-xs font-black uppercase tracking-wider text-[#0284C7] block font-display">
                                  ...
                                  {(item.policyRef || item.durationTag) && (
                                    <div className={`pt-2 border-t ${isPermanent ? 'border-rose-200/60' : 'border-slate-200/60'} flex items-center justify-between text-[11px] sm:text-xs text-slate-400 font-semibold`}>
                                      ...
                                      {item.durationTag && (
                                        <span className={isPermanent ? 'text-rose-600 font-bold' : 'text-[#0284C7] font-bold'}>
                                          {item.durationTag}
                                        </span>
                                      )}
                                    </div>
                ...
                <div className="space-y-4 sm:space-y-5">
                  ...
                  <div className="space-y-3 sm:space-y-3.5">
                    {planData.mustKnow?.items.map((item) => (
                      <div
                        key={item.id}
                        className="must-know-card border border-[#0EA5E9]/20 bg-[#F0F9FF]/40 space-y-2 sm:space-y-2.5 shadow-2xs"
                      >
              ...
              {(activeModal === 'bestSuitedFor' || activeModal === 'perfectFor') && (
                <div className="space-y-4 sm:space-y-5">
                  <div className="pr-8">
                    ...
                    <p className="text-xs text-[#00A8B5] font-medium mt-0.5">
                  ...
                  {((planData.bestSuitedFor?.profiles || planData.perfectFor?.items || []).length > 0) ? (
                    <div className="space-y-3">
                      {(planData.bestSuitedFor?.profiles || planData.perfectFor?.items || []).map((profile, idx) => (
                        <div
                          key={profile.id || idx}
                          className="p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-white border border-[#00A8B5]/30 shadow-2xs space-y-2 text-left hover:border-[#00A8B5]/60 transition-colors"
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {profile.badge && (
                              <span className="inline-block text-[9px] sm:text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#E6F7F8] text-[#00A8B5] border border-[#00A8B5]/20 shrink-0 font-display">
                                {profile.badge}
                              </span>
                            )}
                          </div>
                            ...
                            <ul className="space-y-1 pt-1.5 border-t border-slate-100">
                              {profile.highlights.map((hl, hlIdx) => (
                                <li key={hlIdx} className="flex items-start gap-1.5 text-xs text-slate-700 font-medium">
                                  <FiCheck className="text-[#00A8B5] mt-0.5 shrink-0 text-xs" />
                                  <span>{hl}</span>
                                </li>
                              ))}
                            </ul>
